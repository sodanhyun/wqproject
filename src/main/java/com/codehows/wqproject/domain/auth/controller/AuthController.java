...
public class AuthController {
    ...
    public ResponseEntity<?> tempToken() {
        try{
            HashMap<String, Object> result = authServiceImpl.tempSignin();
            TokenDto tokenDto = (TokenDto)(result.get("tokenDto"));
            HttpHeaders httpHeaders = new HttpHeaders();
            httpHeaders.add(HEADER_AUTHORIZATION, "Bearer " + tokenDto.getAccessToken());
    ...
    public ResponseEntity<?> authorize(@Valid LoginDto loginDto) {
        try{
            TokenResponse tokenResponse = authServiceImpl.signin(loginDto);
            TokenDto tokenDto = (TokenDto)(result.get("tokenDto"));
            HttpHeaders httpHeaders = new HttpHeaders();
            httpHeaders.add(HEADER_AUTHORIZATION, "Bearer " + tokenDto.getAccessToken());
            return new ResponseEntity<>(result, httpHeaders, HttpStatus.OK);
    ...
    public ResponseEntity<?> refresh(@RequestBody TokenDto tokenDto) {
        log.info("refresh in");
        try{
            return ResponseEntity.ok().body(authServiceImpl.refresh(tokenDto));
        }catch (RuntimeException e) {
            return new ResponseEntity<>(e.getMessage(), HttpStatus.METHOD_NOT_ALLOWED);
        }
    }
